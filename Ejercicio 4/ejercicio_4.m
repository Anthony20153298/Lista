function ejercicio_4()
to=0; % start time
tf=10; % end time
t=linspace(to,tf,200);
xo(1)=-1; % initial condition
xo(2)=3;
%u=1;
global fileID
fileID = fopen('datos4.txt','w');
options1=odeset('RelTol',0.01);
[t,x] = ode23(@(t,x)nonlinear(t,x),t,xo,options1);
%[T,Y] = ode45(@(t,y) test(t,y,b), tspan, y0);  % One comma removed
fclose(fileID);
xlabel('Time (s)')
ylabel('Amplitude')
hold on
plot(t,x(:,1))
plot(t,x(:,2))
plot(t,u,'y')
legend('x1','x2', 'u');
grid
function [xdot] = nonlinear(t,x)
% nonlinear model to integrate with ODE45
% parameters
% deslizante control
global fileID
%c=1+rand
cmin=2;
cmax=2;
c=sqrt(cmin*cmax);
a=2.5;
b=2;
%c=1.79;
eta=1;
w=0.25*pi;
xd=sin(w*t);
xd1=cos(w*t)*w;
xd2=-sin(w*t)*w^2;
lamda=1;
F=(0.5*abs(x(1))*x(2)^2+2*x(1)^3*cos(2*x(1)));
u1=(1/c)*(-2.5*abs(x(1))*x(2)^2+xd2-lamda*(x(2)-xd1));
k=((sqrt(cmax/cmin)*(F+eta))+((sqrt(cmax/cmin))-1)*abs(u1));
s=x(2)-xd1+lamda*(x(1)-xd);
u=(1/sqrt(cmax/cmin))*(u1-k*sign(s));
xdot=[x(2);-a*abs(x(1))*x(2)^2-b*x(1)^3*cos(2*x(1))+c*u];
fprintf(fileID,'%10.6f %10.6f %10.6f %10.6f %10.6f \n',t,u,x(1),x(2),xd);